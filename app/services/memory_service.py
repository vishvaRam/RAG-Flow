...
class ConversationMemoryService:
    ...
    async def get_context_for_llm(self, session_id: str) -> ConversationContext:
        ...
        try:
            (
                ...
                recent_messages,
                total_count,
            ) = await db_service.get_conversation_context(session_id, self.window_size)

            # Format recent messages for LLM
            formatted_messages = [
                {"role": msg.sender_type, "content": msg.message}
            ...
            duration = (time.time() - start_time) * 1000
            logger.error(
                f"❌ Failed to get context for session {session_id}: {e}", exc_info=True
            )
            logger.error(f"   ⏱️  Failed after {duration:.2f}ms")

            # Return empty context on error
            return ConversationContext(
                summary=None, recent_messages=[], total_messages=0
            )
        ...
        try:
            ...
            if summary_obj is None:
                ...
                if should_summarize:
                    ...
                    logger.debug(
                        f"⏸️  Summarization not needed: {total_count}/{self.summarize_threshold} messages"
                    )
                ...
                if should_summarize:
                    ...
                    logger.debug(
                        f"⏸️  Summarization not needed: {messages_since_summary}/{self.summarize_threshold} new messages"
                    )
            ...
            duration = (time.time() - start_time) * 1000
            logger.error(
                f"❌ Error checking summarization for session {session_id}: {e}",
                exc_info=True,
            )
            logger.error(f"   ⏱️  Failed after {duration:.2f}ms")
            return False
    ...
    async def generate_summary_prompt(self, session_id: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"   • Has existing summary: {'Yes' if existing_summary else 'No'}"
            )

            # Get the messages that need summarization
            messages = await db_service.get_chat_history(
                session_id, limit=messages_to_summarize_count, offset=0
            )
            ...
            conversation_text = "\n".join(
                [f"{msg.sender_type.upper()}: {msg.message}" for msg in messages]
            )
            ...
            duration = (time.time() - start_time) * 1000
            logger.error(
                f"❌ Error generating summary prompt for session {session_id}: {e}",
                exc_info=True,
            )
            logger.error(f"   ⏱️  Failed after {duration:.2f}ms")
            raise
    ...
    def build_context_prompt(
        self, context: ConversationContext, current_query: str
    ) -> str:
        """
        ...
        try:
            ...
            logger.info(
                f"   • Components: {'Summary' if context.summary else ''}"
