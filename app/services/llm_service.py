...
class LLMService:
    ...
    async def rewrite_query(
        ...
        try:
            ...
            logger.warning(
                f"Query rewriting timeout after {settings.QUERY_REWRITE_TIMEOUT}s"
            )
            return query
    ...
    async def rewrite_query_with_history(
        ...
        if context and context.recent_messages:
            try:
                ...
                context_parts = [
                    f"{msg['role']}: {msg['content'][:100]}" for msg in messages_to_use
                ]
        ...
        try:
            ...
            logger.warning(
                f"⏱️  Query rewriting timeout after {settings.QUERY_REWRITE_TIMEOUT}s"
            )
            return query
        except Exception as e:
    ...
    async def generate_summary(
        ...
        try:
            ...
            logger.info(
                f"🔄 Starting background summarization for session {session_id}"
            )
            ...
            logger.info(
                f"✅ Background summarization completed for session {session_id}"
            )
